...
app.listen(3900,()=>console.log('Express server is running  at port 3900'));
...
app.get('/casedetails',(req,res)=>{
    mysqlConnection.query('SELECT * FROM casedetails',(err, rows, fields)=>{
        if(!err)
        res.send(rows);
        else
        console.log(err);
...
app.delete('/casedetails/:id',(req,res)=>{
    mysqlConnection.query('DELETE FROM assignments WHERE ID = ?',[req.params.id],(err, rows, fields)=>{
...
//Update postjobs id 
app.put('/casedetails',jsonParser,(req,res)=> {
    let emp = req.body;
    var sql = 'UPDATE caseverificationdb.casedetails SET CaseID=?, Description=?, InsurerVerificationNotes=?, ThirdVerificationNotes=?, CreatedBy=?, CreatedDate=?, LastModifiedBy=?, LastModifiedDate=?  WHERE ID = ?';
    mysqlConnection.query(sql, [emp.CaseID,emp.Description,emp.InsurerVerificationNotes,emp.ThirdpartyVerificationNotes,emp.CreatedBy,emp.CreatedDate,emp.LastModifiedBy,emp.LastModifiedDate,emp.ID],(err, result)=>{
        if(!err)
            res.json(result);
        else
        console.log(err);
